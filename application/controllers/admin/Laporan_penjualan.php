...
class Laporan_penjualan extends CI_Controller
{
	...
	public function pertanggal($tanggal = null)
	{
		$this->load->model('transaksi_sales_m');
		if ($tanggal == null) {
			$laporan_penjualan = $this->transaksi_sales_m->read_pertanggal()->result_array();
		} else {
			$laporan_penjualan = $this->transaksi_sales_m->read_full_where(array('date(transaksi_sales.created_at)' => $tanggal))->result_array();
		}
		$data = array(
			'js' => 'admin/js_laporan_penjualan',
			'parsing' => array('laporan_penjualan' => $laporan_penjualan, 'total' => count($laporan_penjualan))
		);
		if ($tanggal == null) {
			$data['konten'] = 'admin/laporan_pertanggal';
		} else {
			$data['konten'] = 'admin/laporan_pertransaksi';
		}
		$this->load->view('_partials/template', $data);
	}
	...
	public function export_v2_where()
	{
		...
		$spreadsheet->setActiveSheetIndex(0)
			...
			->setCellValue('M' . $kolom, $grand_total);
	...
	public function edit_tts($id_transaksi_sales)
	{
		$this->load->model('barang_m');
		$this->load->model('toko_m');
		$this->load->model('pembayaran_m');
		$this->load->model('transaksi_sales_m');
		$this->load->model('item_transaksi_m');
		$transaksi = $this->transaksi_sales_m->read_where(array('id_transaksi_sales' => $id_transaksi_sales))->row_array();
		$pembayaran = $this->pembayaran_m->read_where(array('id_transaksi_sales' => $id_transaksi_sales))->row_array();
		//echo var_dump($pembayaran);die();
		$data = array(
			'konten' => 'admin/form_tts',
			'parsing' => array(
				'transaksi' => $transaksi,
				'pembayaran' => 'Rp. ' . number_format($pembayaran['jumlah_pembayaran'], 0, ',', '.'),
				'id_transaksi_sales' => $id_transaksi_sales,
			),
			'js' => 'admin/js_tts',
		);
	...
	public function aksi_edit_tts($id_transaksi_sales)
	{
		$this->load->model('barang_m');
		$this->load->model('toko_m');
		$this->load->model('pembayaran_m');
		...
		if (
			$this->transaksi_sales_m->update(array(
			...
			$this->session->set_flashdata(array(
				'status' => 1,
				'message' => 'Berhasil Transfer TTS',
			));
			redirect(base_url('admin/laporan_penjualan/detail_pending/').$id_transaksi_sales);
		} else {
			$this->session->set_flashdata(array(
				'status' => 0,
				'message' => 'Gagal Transfer TTS',
			));
			redirect(base_url('admin/laporan_penjualan/edit_tts/').$id_transaksi_sales);
		}
	}
