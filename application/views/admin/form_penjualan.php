    ...
    foreach ($this->cart->contents() as $data) {?>
								...
								<div class="form-group">
									...
									<select name="id_toko" class="form-control" tabindex="-1" style="width: 100%" required>
										<option value="0" disabled selected>Pilih Toko</option>
										<?php foreach ($parsing['toko'] as $data) {?>
											<option value="<?=$data['id_toko']?>" <?php if ($data['id_toko'] == $parsing['toko_dipilih']) {echo 'selected';}?>><?=$data['nama_toko']?> - <?=$data['alamat']?></option>
										<?php }?>
								...
								<div class="form-group">
									...
									<div class="input-group-prepend">
										<!-- <span class="input-group-text" id="inputGroupPrepend">Rp</span> -->
										<input name="pembayaran" type="text" class="form-control" onkeyup="rupiah(this)" id="pembayaran" placeholder="Jumlah yang dibayar" value="<?=$parsing['pembayaran']?>" required>

									</div>
								</div>
								<button type="submit" class="btn btn-block btn-primary">Edit Transaksi</button>
